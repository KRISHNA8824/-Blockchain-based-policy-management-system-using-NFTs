...
const contractAddress = '0x818ddF70Ff1db1d3Db70b68F32aFfe3E094A6718'; // Replace with your contract address
...
contract.methods.mint(recipientAddress, tokenURI).send({
  from: '0x22fd564D0908bcc7fa393171675675e9afFfd22d',
  gasPrice: '0'
})
.then((receipt) => {
...
function App() {
  ...
  const mintToken = async () => {
    ...
    // Call the mint function on the contract to mint a new token
    try {
      const receipt = await contract.methods.mint(recipientAddress, tokenURI).send({
...
function App() {
  ...
  const getTokensOwned = async () => {
    ...
    // Loop through each token and get its ID
    const tokens = [];
    ...
    // Set the list of tokens owned by the account
    setTokensOwned(tokens);
  }

  return (
    <div>
      <input type="text" placeholder="Account address" onChange={(e) => setAccountAddress(e.target.value)} />
